...
predictor_all = scaling(data_inp(:,1:psize));
predictor = [predictor_all(:,2), predictor_all(:,3), predictor_all(:,4), predictor_all(:,8)];
response = data_inp(:,psize+1);

rep = 10000;

t_er = zeros(9,10,'double');
...
t_p = [t_param(:,1), t_param(:,2+1), t_param(:,3+1), t_param(:,4+1), t_param(:,8+1)];
t_param = t_p';
